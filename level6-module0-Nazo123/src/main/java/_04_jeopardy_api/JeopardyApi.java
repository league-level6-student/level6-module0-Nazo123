...
public class JeopardyApi {
    ...
    public Clue getClue(int value) {
    	...
    	Clue[] c = webClient.get().uri(uriBuilder -> uriBuilder.queryParam("value", value).build()).retrieve().bodyToMono(Clue[].class).block();
        //2
        //Get a random number less than the size of the Clue array
    	int ranInd = new Random().nextInt(c.length+1);
        //3
